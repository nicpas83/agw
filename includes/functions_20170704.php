...
function tabla_reporte_inspecciones($perfil, $filtroPerfilId=null, $desde=null, $hasta=null, $planta=null, $filtroDin=null)
{
    ...
    */
    if($perfil == "adm"){
        ...
        $cant_w_us = "";
        $lista_warrant_us = "";
        $filtroUsuario = "WHERE ACTA_NRO IS NOT NULL";
        $filtroReporte .= " AND t1.ACTA_FECHA > DATE_SUB(curdate(), INTERVAL 1 YEAR)"; 
    ...
    if(!empty($desde) AND !empty($hasta)){
        ...
        $filtroReporte .= " AND t1.ACTA_FECHA between '$desde' AND '$hasta'"; 
    ...
    $sql = "
        ...
        FROM
            (SELECT 
                ...
                ACTA_PLANTA_ID,
                ACTA_NRO,
                CONCAT(INSP_NOMBRE,', ',INSP_APELLIDO) AS INSPECTOR,
                PLAN_RASO,
                PLAN_NOMBRE,
                LOCA_NOMBRE,
                ACTA_MOTIVO
            FROM actas
            INNER JOIN inspectores ON ACTA_INSPECTOR_ID = INSP_ID
            INNER JOIN plantas ON ACTA_PLANTA_ID = PLAN_ID
        ...
        INNER JOIN 
            ...
            FROM actas_almacenes_qx INNER JOIN productos ON AAQX_PRODUCTO_ID = PROD_ID
            GROUP BY AAQX_ACTA_NRO, PROD_NOMBRE
            ) t2
        ...
        LEFT JOIN
            ...
            ON TITU_PRODUCTO = PROD_NOMBRE
            INNER JOIN depositantes
            ON TITU_DEPO_ID = DEPO_ID
            RIGHT OUTER JOIN 
            (SELECT
                ACTA_FECHA,
                ...
                AAQX_PRODUCTO_ID
            FROM actas
            INNER JOIN actas_almacenes_qx
            ON ACTA_NRO = AAQX_ACTA_NRO
            INNER JOIN almacenes 
            ON AAQX_ALMA_ID = ALMA_ID
            GROUP BY ACTA_FECHA, ALMA_PLAN_ID, AAQX_PRODUCTO_ID
            ORDER BY ACTA_FECHA DESC
            ) sub
        ON PROD_ID = sub.AAQX_PRODUCTO_ID AND TITU_PLANTA_ID = sub.ALMA_PLAN_ID

        WHERE TITU_FECHA_EMISION < sub.ACTA_FECHA AND (TITU_FECHA_LIBERACION IS NULL OR TITU_FECHA_LIBERACION > sub.ACTA_FECHA)
        GROUP BY sub.ACTA_FECHA, TITU_PLANTA_ID, PROD_NOMBRE
        ORDER BY sub.ACTA_FECHA DESC
        ) t3
        ON t1.ACTA_FECHA = t3.ACTA_FECHA AND t1.ACTA_PLANTA_ID = t3.TITU_PLANTA_ID

        $filtroUsuario
        $filtroReporte
        $filtroHaving
            
        ORDER BY t1.ACTA_FECHA DESC
